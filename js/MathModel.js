...
(function() {
  ...
  ParametricPathModel = (function(superClass) {
    extend(ParametricPathModel, superClass);
    ...
    ParametricPathModel.prototype.generate = function() {
      var geometry, i, j, ref, t;
      geometry = new THREE.Geometry();
      for (i = j = 1, ref = this.resolution; 1 <= ref ? j <= ref : j >= ref; i = 1 <= ref ? ++j : --j) {
  ...
  IsoCurve = (function(superClass) {
    extend(IsoCurve, superClass);
    ...
    function IsoCurve(arg) {
      this.func = arg.func, this.xmin = arg.xmin, this.xmax = arg.xmax, this.ymin = arg.ymin, this.ymax = arg.ymax, this.resolution = arg.resolution, this.smoothingLevel = arg.smoothingLevel, this.material = arg.material, this.name = arg.name, this.algorithm = arg.algorithm;
      if (this.xmin == null) {
        this.xmin = -3.00;
      }
      if (this.xmax == null) {
    ...
    IsoCurve.prototype.march_async = function(b, algorithm) {
      var blob, debug, e, error, f, mc, response, that, worker;
      if (algorithm == null) {
  ...
  MarchingCubesModel = (function(superClass) {
    extend(MarchingCubesModel, superClass);
    ...
    function MarchingCubesModel(arg) {
      this.func = arg.func, this.xmin = arg.xmin, this.xmax = arg.xmax, this.ymin = arg.ymin, this.ymax = arg.ymax, this.zmin = arg.zmin, this.zmax = arg.zmax, this.resolution = arg.resolution, this.smoothingLevel = arg.smoothingLevel, this.material = arg.material, this.name = arg.name, this.algorithm = arg.algorithm;
      this.embedCallback = bind(this.embedCallback, this);
      this.embedObjects = bind(this.embedObjects, this);
      if (this.xmin == null) {
        this.xmin = -3.00;
      }
      if (this.xmax == null) {
  ...
  VectorModel = (function(superClass) {
    extend(VectorModel, superClass);
    ...
    function VectorModel(arg) {
      var color, dir, origin, vector;
      origin = arg.origin, vector = arg.vector, color = arg.color;
      if (origin == null) {
        origin = [0, 0, 0];
      }
  ...
  PlaneShadowModel = (function(superClass) {
    extend(PlaneShadowModel, superClass);
    ...
    function PlaneShadowModel(arg) {
      var color, normal, position, s, xrange, yrange;
      normal = arg.normal, position = arg.position, xrange = arg.xrange, yrange = arg.yrange, color = arg.color;
      if (position == null) {
        position = [0, 0, 0];
      }
      if (normal == null) {
